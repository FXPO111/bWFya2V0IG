...
class AdvancedMarketMaker:
    ...
    def __init__(self, agent_id: str, capital: float):
        ...
        self.max_spread_ticks: float = 2.5   # CALIB: EUR calm = ~2 ticks stressed
    ...
    def _compute_spread_and_skew(self, mid: float) -> (float, float, float):
        ...
        # CALIB: EUR/USD target spread ~0.45 TICK calm. Old base=1.2 → 2x too wide.
        base = 0.45

        # волатильность
        base += max(0.0, (sig1 - 3.0) / 70.0)
        if sig10 > 0.0:
            slope = abs(sig1 - sig10) / sig10
            base += min(0.25, slope * 0.25)

        # alpha / дисбаланс
        base += 0.12 * min(1.5, abs(sa) / 5.0)
        base += 0.12 * min(1.5, abs(am) / 8.0)
        base += 0.15 * abs(ofi)
        base += 0.12 * abs(cl)

        # токсичность / pnl-variance
        base += 0.30 * tox
        base += min(0.40, math.sqrt(self.pnl_var_ema.value) / (0.006 * self.capital + 1e-6))
        ...
        # CALIB: floor 0.35 TICK (was 0.8) — eliminates locked-book states
        base = max(0.35, min(self.max_spread_ticks, base))
        ...
        # CALIB: per-side floor 0.35 (was 0.8)
        bid_sp = max(0.35, min(self.max_spread_ticks + 0.5, bid_sp))
        ask_sp = max(0.35, min(self.max_spread_ticks + 0.5, ask_sp))
    ...
    def _size_for_level(self, px: float, mid: float, spread_ticks: float, risk: float, near: bool, level: int) -> (float, bool, Dict[str, Any]):
        ...
        base_notional_far = self.capital * 0.0008
    ...
    def _levels_from_mid(self, mid: float, bid_spread: float, ask_spread: float) -> List[tuple]:
        """
    ...
    def generate_orders(self, *args, **kwargs) -> List[Order]:
        ...
        levels = self._levels_from_mid(mid, bid_spread, ask_spread)
